/* =============================================================================
   timeline.js — the multi-year view: one lane per system across the whole
   window, markers at their true fractional position.

   Marker placement is NOT decided here. `f` is assigned once by placeEvents()
   in main.js, and both this view and the ring read it, which is what keeps the
   two from drifting apart. This file only paints.

   Was: lines 552-601 of data_donut_concept_4_1.html.
   ============================================================================= */

import {
  sysOrder, COL, NAME, SHORT, LBL_FS, months, year, SPAN,
...
/* Name the month an event belongs to. Events sharing a lane and a month are
   spread evenly across that month's cell, so the tooltip says which slot of
   how many — that spacing is ordering, not a day of the month. */
function whenLabel(e) {
  const txt = monthLabel(absMonth(e));
...
export function drawTimeline(events) {
  ...
  const tl = Object.fromEntries(
    sysOrder.map(k => [k, events.filter(e => e.sys === k)])
  );
  ...
  sysOrder.forEach(sys => {
    ...
    if (!evs.length) track.innerHTML = '<span>lane added — awaiting milestones</span>';

    evs.forEach(ev => {
      const m = document.createElement('div');
      m.className = 'milestone';
      m.dataset.pct = (ev.f * 100);          // true position, kept for re-layout
      m.dataset.when = whenLabel(ev);
